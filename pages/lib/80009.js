...
export function materializePolygon(polygon, bars) {
...
export function dematerializePolygon(polygon, id, barIndices) {
  return {
    id: id,
    points: polygon.points.map((point) => ({
      ...point,
...
export function isPolygonInBarsRange(polygon, bars) {
  if (
    polygon.points.some((point) =>
      bars.contains(point.index + (point.offset || 0))
    )
  )
    return true;
