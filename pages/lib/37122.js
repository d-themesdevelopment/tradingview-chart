...
import { Point } from "lib/point"; // ! not correct
...
class LineToolFibWedgeBase extends LineDataSource {
      ...
      if (!this._recursiveGuard) {
        try {
          ...
          if (index === 2) {
            ...
            let middlePoint = ensureNotNull(
              this.pointToScreenPoint(this._points[1])
            );
            const endPoint = ensureNotNull(
            ...
            const screenPoint = ensureNotNull(
              this.screenPointToPoint(middlePoint)
            );
            const priceProperty = this._pointsProperty
              .childs()
              .points[1].childs().price;
            const barProperty = this._pointsProperty
              .childs()
              .points[1].childs().bar;
            ...
            const middlePoint = ensureNotNull(
              this.pointToScreenPoint(this._points[1])
            );
            let endPoint = ensureNotNull(
              this.pointToScreenPoint(this._points[2])
            );
            ...
            const screenPoint = ensureNotNull(
              this.screenPointToPoint(endPoint)
            );
            const priceProperty = this._pointsProperty
              .childs()
              .points[2].childs().price;
            const barProperty = this._pointsProperty
              .childs()
              .points[2].childs().bar;
            priceProperty.setValue(screenPoint.price);
  ...
  addPoint(point) {
    if (this._points.length === 2) {
      ...
      const price = ensureNotNull(this.priceScale()).coordinateToPrice(
        endPoint.y,
        firstValueData
      );
      point = {
        index: Math.round(
          this._model.timeScale().coordinateToIndex(endPoint.x)
        ),
        price: price,
      };
    }
