import { box, Point } from 'path2d';
import { colorFromBackground } from 'some-module';
import { lastMouseOrTouchEventInfo } from 'some-module';
...
class PriceAxisView {
  constructor(renderer) {
    ...
    this.axisRenderer = new PriceAxisRenderer(this.axisData, this.commonRendererData);
    this.paneRenderer = new PriceAxisRenderer(this.paneData, this.commonRendererData);
  }
  ...
  getFloatCoordinate() {
    this.updateRendererDataIfNeeded();
    return this.commonRendererData.floatCoordinate ?? this.commonRendererData.coordinate;
  }
  ...
  updateRendererDataIfNeeded() {
    if (this.invalidated) {
      ...
      this.updateRendererData(this.axisData, this.paneData, this.commonRendererData);
...
class PriceAxisRenderer {
  ...
  draw(context, options, coordinateConverter, pixelRatio, minVisible, maxVisible, logicalRange) {
    const data = this.data;
    if (!data.visible || this.isOutOfScreen(options, minVisible, maxVisible)) return;
    ...
    const paddingBottom = options.paddingBottom + commonData.additionalPaddingBottom;
    const paddingInner = options.paddingInner;
    const additionalPaddingInner = data.ignoreAdditionalPaddingInner ? 0 : options.additionalPaddingInner;
    ...
    const yMidCorrection = coordinateConverter.yMidCorrection(context, text) * pixelRatio;
    ...
    const secondLineWidth = hasSecondLine ? Math.ceil(coordinateConverter.measureText(context, secondLine)) : 0;
    const thirdLineWidth = hasThirdLine ? Math.ceil(coordinateConverter.measureText(context, thirdLine)) : 0;

    const contentWidth = Math.max(
      borderSize + paddingInner + paddingOuter + textWidth + additionalPaddingInner,
      borderSize + paddingInner + paddingOuter + secondLineWidth + additionalPaddingInner,
      borderSize + paddingInner + paddingOuter + thirdLineWidth + additionalPaddingInner
    );

    const lineHeightRounded = Math.round(lineHeight * pixelRatio);
    const lineHeightHalfRounded = lineHeightRounded % 2 === 0 ? lineHeightRounded / 2 : (lineHeightRounded - 1) / 2;
    const lineHeightFloor = Math.floor(lineHeight * pixelRatio);

    let contentHeight = 0;
    if (text || isIconVisible) {
      contentHeight = hasThirdLine ? lineHeightRounded * 2 : hasSecondLine ? lineHeightRounded : lineHeightRounded;
    }

    const borderWidth = data.borderVisible ? Math.floor(borderSize * pixelRatio) : 0;
    ...
    const paddingLeftRightScaled = Math.round((paddingInner + paddingOuter) * pixelRatio);
    const contentWidthScaled = Math.round(contentWidth * pixelRatio);
    const additionalPaddingInnerScaled = Math.round(additionalPaddingInner * pixelRatio);
    ...
    let centerX = Math.round(coordinateConverter.logicToCoordinate(
      logicalRange.min + logicalRangeStep * data.xCoord,
    ) * pixelRatio);

    const centerY = Math.round(coordinateConverter.logicToCoordinate(
      commonData.fixedCoordinate ?? commonData.coordinate,
    ) * pixelRatio) - lineHeightHalfRounded;
    ...
    if (data.text || isIconVisible) {
      ...
      const iconWidth = options.borderSize + options.paddingInner + options.paddingOuter + contentWidthScaled + additionalPaddingInnerScaled;
      ...
      if (data.borderVisible) {
        if (options.alignLabels) {
          drawRoundRectWithInnerBorder(context, centerX - iconWidth / 2, contentTop, iconWidth, contentHeight, data.background, [lineHeightFloor, 0, 0, lineHeightFloor], borderWidthScaled, borderColor, commonData.borderStyle);
        } else {
          drawRoundRectWithInnerBorder(context, contentTop, centerX - iconWidth / 2, contentHeight, iconWidth, data.background, [0, lineHeightFloor, lineHeightFloor, 0], borderWidthScaled, borderColor, commonData.borderStyle);
        }
      }
    ...
    if (data.separatorVisible) {
      ...
      context.fillRect(options.alignLabels ? contentBottom - separatorWidthScaled : 0, contentTop, separatorWidthScaled, contentBottom - contentTop);
    }
  ...
  topBottomTotalHeight(options) {
    ...
    const fontSize = options.fontSize;
    const paddingTop = options.paddingTop + this.commonData.additionalPaddingTop;
    const paddingBottom = (lines - 0.5) * fontSize + (lines - 1) * options.lineSpacing + options.paddingBottom + this.commonData.additionalPaddingBottom;
  ...
  getVisibleLines() {
    const data = this.data;
    return (data.text ? 1 : 0) + (data.secondLine ? 1 : 0) + (data.thirdLine ? 1 : 0);
  }

  isOutOfScreen(options, minVisible, maxVisible) {
    const fixedCoordinate = this.commonData.fixedCoordinate ?? this.commonData.coordinate;
    const { total } = this.topBottomTotalHeight(options);
    const lineHeight = total / this.getVisibleLines();

    return fixedCoordinate - lineHeight / 2 - 3 > maxVisible.y || fixedCoordinate + (total - lineHeight / 2) + 3 < minVisible.y;
  }
}

const drawRoundRectWithInnerBorder = (context, x, y, width, height, backgroundColor, innerRadii, borderWidth, borderColor, borderStyle) => {
  context.fillStyle = backgroundColor;
  const [topLeftRadius, topRightRadius, bottomRightRadius, bottomLeftRadius] = innerRadii;

  context.beginPath();
  context.moveTo(x + topLeftRadius, y);
  context.lineTo(x + width - topRightRadius, y);
  context.arc(x + width - topRightRadius, y + topRightRadius, topRightRadius, -Math.PI / 2, 0);
  context.lineTo(x + width, y + height - bottomRightRadius);
  context.arc(x + width - bottomRightRadius, y + height - bottomRightRadius, bottomRightRadius, 0, Math.PI / 2);
  context.lineTo(x + bottomLeftRadius, y + height);
  context.arc(x + bottomLeftRadius, y + height - bottomLeftRadius, bottomLeftRadius, Math.PI / 2, Math.PI);
  context.lineTo(x, y + topLeftRadius);
  context.arc(x + topLeftRadius, y + topLeftRadius, topLeftRadius, Math.PI, Math.PI + Math.PI / 2);
  context.closePath();
  context.fill();
