...
const {
  Pattern5pointsPaneView: Pattern5PointsPaneView,
} = require("some-module"); // ! not correct
const { LINESTYLE_DOTTED, LINESTYLE_SOLID } = require("./79849");
const { ChartFontFamily } = require("some-module-fonts"); // ! not correct
...
class Pattern5PointsPaneView extends Pattern5PointsPaneView {
  constructor(e, t) {
    ...
    this._polylineRenderer = new PolygonRenderer(
  ...
  _updateImpl() {
    ...
    if (this._points.length >= 3) {
      const xbLabel = textStyle(
        pointA.add(pointC).scaled(0.5),
    ...
    if (this._points.length >= 4) {
      ...
      const acLabel = textStyle(
        pointB.add(pointD).scaled(0.5),
    ...
    if (this._points.length >= 5) {
      const bdLabel = textStyle(
        pointA.add(pointE).scaled(0.5),
    ...
    if (points.length >= 3) {
      ...
      this._abRetracement =
        Math.round(
          1000 *
            Math.abs(
              (pointC.price - pointB.price) / (pointB.price - pointA.price)
            )
        ) / 1000;
    }
    ...
    if (points.length >= 4) {
      ...
      this._bcRetracement =
        Math.round(
          1000 *
            Math.abs(
              (pointD.price - pointC.price) / (pointC.price - pointB.price)
            )
        ) / 1000;
    }
    ...
    if (points.length >= 5) {
      ...
      this._cdRetracement =
        Math.round(
          1000 *
            Math.abs(
              (pointE.price - pointD.price) / (pointD.price - pointC.price)
            )
        ) / 1000;
      this._xdRetracement =
        Math.round(
          1000 *
            Math.abs(
              (pointE.price - pointB.price) / (pointB.price - pointA.price)
            )
        ) / 1000;
    }
  }
