...
class Animation {
  ...
  _animate(startTime, currentValue, currentTime) {
    ...
    const elapsedTime =
      (currentTime =
        !currentTime || currentTime < 1e12 ? performance.now() : currentTime) -
      startTime;
    const animationFinished =
      elapsedTime >= this._options.duration ||
      currentValue === this._options.to;
...
export function doAnimate(options) {
