var s = require("./86441"); // ! not correct

import { ensureNotNull } from "./assertions";
import { LineDataSource } from "./13087";
import { LineToolWidthsProperty } from "./68806";
import { createPropertiesObject } from "./CustomLevelsProperty";
class GannSquareFixedLineTool extends LineDataSource {
  constructor(e, t, s, r) {
  ...
  static createProperties(e) {
    const t = createPropertiesObject("linetoolgannfixed", e);
  ...
  static _configureProperties(e) {
    ...
    e.addChild("linesColors", new LineToolColorsProperty(lineColors));
    e.addChild("linesWidths", new LineToolWidthsProperty(lineWidths));
  }
  ...
  axisPoints() {
    ...
    return [points[0], ensureNotNull(this.screenPointToPoint(screenPoints[1]))];
  }
  ...
  getScreenPoints() {
    ...
    const startScreenPoint = ensureNotNull(this.pointToScreenPoint(startPoint));
    ...
    const screenPoints = [
      startScreenPoint.addScaled(directionVector, length),
      startScreenPoint.add(
        new s.Point(5 * length * xDirection, 5 * length * yDirection)
      ),
    ];
    return screenPoints;
  }
  ...
  fanLines() {
    ...
    for (let i = 0; i < fanLinesCount; i++) {
      const fanLineChildProperties = childProperties.fanlines
        .childs()
        [i].childs();
      fanLines.push({
        index: i,
  ...
  _calcAngle() {
    ...
    let angle = Math.acos(directionVector.x);
