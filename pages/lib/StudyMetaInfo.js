...
class StudyMetaInfo extends StudyMetaInfoBase {
  ...
  static versionOf(e) {
    ...
    return (
      t < 0 &&
        loggerStudyMetaUInfo.logError(
          "Metainfo format version cannot be negative: " + t
        ),
      t
    );
  }
    ...
    if (-1 === e.indexOf("@"))
      ...
      if (3 === s.length)
        (t.packageId = s.slice(0, 2).join("-")),
          ...
          (t.version = s[2]);
      else if (1 === s.length && "decisionbar" === s[0])
        (t.packageId = "les-" + s[0]),
          (t.id = t.shortId + "@" + t.packageId),
          (t.version = 1);
      else {
  ...
  static getStudyIdWithLatestVersion(e) {
    ...
    let i = t;
    return (
      t.indexOf("@tv-scripting") >= 0
        ? (i += "-101!")
        : t.endsWith("CP@tv-basicstudies")
        ? (i += "-" + Math.min(e.version, 207))
        : t.endsWith("CP@tv-chartpatterns")
  ...
  static overrideDefaults(e) {
    0 !== e.length &&
      applyOverridesToStudyDefaults(emptyElementStudyMetaInfo, e, function (e) {
        return (
          TradingView.defaultProperties[
            StudyMetaInfo.getStudyPropertyRootName(e)
          ] || null
        );
      });
  }
...
(StudyMetaInfo.VERSION_STUDY_ARG_SOURCE = 41),
  ...
  (TradingView.StudyMetaInfo = StudyMetaInfo);
