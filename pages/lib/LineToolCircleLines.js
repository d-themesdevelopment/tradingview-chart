...
class LineToolCircleLines extends LineDataSource {
  constructor(model, priceScale, timeScale, properties) {
    super(model, properties || LineToolCircleLines.createProperties(), priceScale, timeScale);
    import(/* webpackChunkName: "LineToolCircleLinesPaneView" */ '<path_to_LineToolCircleLinesPaneView_module>').then(({ LineToolCircleLinesPaneView }) => {
  ...
  async _getPropertyDefinitionsViewModelClass() {
    return Promise.all([
      import(/* webpackChunkName: "cyclicAndSineLinesPatternDefinitionsViewModel" */ '<path_to_cyclicAndSineLinesPatternDefinitionsViewModel_module>')
    ]).then(i => i[0].CyclicAndSineLinesPatternDefinitionsViewModel);
  }

  static createProperties(properties) {
    const defaultProperty = new DefaultProperty("linetoolcirclelines", properties);
