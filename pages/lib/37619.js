import { LineToolTrading } from 'path/to/LineToolTrading';
import { LineDataSource } from 'path/to/LineDataSource';
import { PositionAveragePriceAxisView } from 'path/to/PositionAveragePriceAxisView';
import { getColorFromProperties, setColorToProperties, getFontFromProperties, setFontToProperties } from 'path/to/utils';
import { DefaultProperty } from 'path/to/DefaultProperty';
import { PriceFormatter } from 'path/to/PriceFormatter';
import { enabled } from 'path/to/flags';
import { sortSourcesPreOrdered } from 'path/to/sortSourcesPreOrdered';
...
class PositionAdapter {
  ...
  setProtectTooltip(protectTooltip) {
    this._line.properties().protectTooltip.setValue(ensureString(protectTooltip));
    return this;
  }
  ...
  setReverseTooltip(reverseTooltip) {
    this._line.properties().reverseTooltip.setValue(ensureString(reverseTooltip));
    return this;
  }
  ...
  getExtendLeft() {
    ...
    if (extendLeft === 'inherit') {
      extendLeft = this._line._model.properties().tradingProperties.extendLeft.value();
    }
    return extendLeft;
  }
  ...
  getLineLength() {
    ...
    if (lineLength === 'inherit') {
      lineLength = this._line._model.properties().tradingProperties.lineLength.value();
    }
    return lineLength;
  }

  setLineLength(lineLength) {
    this._line.properties().lineLength.setValue(Math.max(0, Math.min(lineLength, 100)));
    return this;
  }

  getLineColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.lineBuyColor.value() : properties.lineSellColor.value();
  }
  ...
  getLineStyle() {
    ...
    if (lineStyle === 'inherit') {
      lineStyle = this._line._model.properties().tradingProperties.lineStyle.value();
    }
    return lineStyle;
  }
  ...
  getLineWidth() {
    ...
    if (lineWidth === 'inherit') {
      lineWidth = this._line._model.properties().tradingProperties.lineWidth.value();
    }
    return lineWidth;
  }
  ...
  getBodyBorderColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.bodyBorderBuyColor.value() : properties.bodyBorderSellColor.value();
  }
  ...
  getQuantityBorderColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.quantityBorderBuyColor.value() : properties.quantityBorderSellColor.value();
  }
  ...
  getQuantityBackgroundColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.quantityBackgroundBuyColor.value() : properties.quantityBackgroundSellColor.value();
  }
  ...
  getReverseButtonBorderColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.reverseButtonBorderBuyColor.value() : properties.reverseButtonBorderSellColor.value();
  }
  ...
  getReverseButtonIconColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.reverseButtonIconBuyColor.value() : properties.reverseButtonIconSellColor.value();
  }
  ...
  getCloseButtonBorderColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.closeButtonBorderBuyColor.value() : properties.closeButtonBorderSellColor.value();
  }
  ...
  getCloseButtonIconColor() {
    const properties = this._line.properties();
    return this._direction === 'buy' ? properties.closeButtonIconBuyColor.value() : properties.closeButtonIconSellColor.value();
  }
...
class PositionLineTool extends LineToolTrading {
  constructor(line, model) {
    ...
    this._formatter = new PriceFormatter(priceScale, minMove, fractional, minMove2);
  }
  ...
  paneViews() {
    return enabled('snapshot_trading_drawings') ? null : this._model.properties().tradingProperties.showPositions.value()
      ? super.paneViews()
      : null;
  }
  ...
  static createProperties(properties) {
    const defaultProperties = new DefaultProperty('linetoolposition', properties, false, false);
