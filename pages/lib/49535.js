  ...
  },
  {
    ...
    ctor: class extends PriceScaleSelectionStrategy {
      ...
      apply(pane) {
        ...
        while (pane.rightPriceScales().length - pane.leftPriceScales().length > 
...
function createPriceScaleSelectionStrategy(name) {
  const selectedStrategy = ensureDefined(allPriceScaleSelectionStrategyInfo.find(strategy => strategy.name === name));
