...
const ContextMenuManager = {
  createAction: (config) =>
    new Action({ ...config, actionId: "Chart.CustomActionId" }),
  createAsyncAction: (config) =>
    new ActionAsync("Chart.CustomActionId", config),
  ...
  showMenu: (items, target, options = {}, callback, callbackArg) => {
    return ContextMenuManager.createMenu(
      items,
      options,
      callback,
      callbackArg
    ).then((menu) => menu.show(target));
  },
