...
import { showOnElement, hide } from "./38780";
...
function hasOverflowInChildren(element, axis = "x") {
  for (const child of Array.from(element.children)) {
    if (
      ...
      (hasOverflow(child, axis) || hasOverflowInChildren(child, axis))
    ) {
      return true;
    }
  }
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (
        ...
        target.matches(".apply-overflow-tooltip")
      ) {
        let axis = "x";
        ...
        if (
          target.matches(".apply-overflow-tooltip--check-children-recursively")
        ) {
        ...
        if (
          target.matches &&
          target.matches(".apply-overflow-tooltip--allow-text")
        ) {
          tooltipText = target.textContent || "";
        }
        const dataTooltipText = target.getAttribute(
        ...
        const removeListeners = () => {
          hide();
          ["mouseleave", "mousedown"].forEach((eventType) =>
            target.removeEventListener(eventType, removeListeners)
          );
        };
        ["mouseleave", "mousedown"].forEach((eventType) =>
          target.addEventListener(eventType, removeListeners)
        );
      }
    },
    true
...
function handleLoginStateChange() {
  window.iframeAuthWidget &&
    !false === window.user.profile_data_filled &&
...
loginStateChangeHandler = window.loginStateChange
  ? window.loginStateChange
  : (window.loginStateChange = new d());
...
TVXWindowEvents.on("signOut", () => {
  if (window.initData.lfs) {
    const adminWarningElement =
      document.getElementsByClassName("js-admin-warning")[0];
