...
const eraseLevelLineTranslation = new TranslatedString("erase level line", t(null, void 0, bind));

class LineToolFibSpeedResistanceFan extends LineDataSource {
  static HLevelsCount = 7;
  static VLevelsCount = 7;

  constructor(data, properties, options, model) {
    super(data, properties || LineToolFibSpeedResistanceFan.createProperties(), options, model);
    loadPromise.then(bind(this, ({ FibSpeedResistanceFanPaneView }) => {
      this._setPaneViews([new FibSpeedResistanceFanPaneView(this, this._model)]);
    }));
  }
  ...
  processErase(source, target) {
    const propertyName = target.type === "h" ? `hlevel${target.index}` : `vlevel${target.index}`;
  ...
  async _getPropertyDefinitionsViewModelClass() {
    return Promise.all([loadPromise, loadPromise1, loadPromise2, loadPromise3, loadPromise4])
  ...
  static createProperties(options) {
    ...
    const levelsProperty = new LevelsProperty("linetoolfibspeedresistancefan", options, false, {
      range: [1, 7],
      prefixes: ["h", "v"],
