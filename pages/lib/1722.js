...
function isInherited(child, parent) {
    if (typeof child !== 'function' || typeof parent !== 'function') {
        throw new TypeError("isInherited: child and parent should be constructor functions");
    }
    return child.prototype instanceof parent || child.prototype === parent.prototype;
}
...
function isHashObject(value) {
    return isObject(value) && value.constructor && value.constructor.toString().indexOf("function Object") !== -1;
}
...
function declareClassAsPureInterface(classConstructor, interfaceName) {
    for (var prop in classConstructor.prototype) {
        if (typeof classConstructor.prototype[prop] === 'function' && classConstructor.prototype.hasOwnProperty(prop)) {
            classConstructor.prototype[prop] = function() {
                throw new Error(interfaceName + "::" + prop + " is an interface member declaration and must be overloaded in order to be called");
            };
        }
    }
}

function requireFullInterfaceImplementation(constructor, interfaceConstructor, interfaceName) {
    for (var prop in interfaceConstructor.prototype) {
        if (typeof interfaceConstructor.prototype[prop] === 'function' && !constructor.prototype[prop]) {
            throw new Error("Interface implementation assertion failed: " + constructor.name + " does not implement " + interfaceName + "::" + prop + " function");
        }
    }
}
