    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
      ...
      if (token instanceof NumberToken || token instanceof SymbolToken) {
        if (incompleteTokenStartIndex !== undefined) {
          ...
          const removedTokens = tokens.splice(
            incompleteTokenStartIndex,
            i - incompleteTokenStartIndex + 1,
            newToken
          );
...
function validate(tokens) {
  ...
  const braces = [];
  var c = { isEmpty: true, warnings: [], errors: [] };
      ...
      if (token instanceof SymbolToken || token instanceof NumberToken) {
        ...
        if (!lastBrace) {
          ...
          if (
            ...
            nextToken.precedence > lastBrace.minPrecedence
          ) {
            errors.push({
              status: "error",
              reason: "unexpected_token",
...
function factorOutBraces(tokens) {
  tokens = tokens.filter(
    (token) =>
      !(token instanceof OpenBraceToken || token instanceof CloseBraceToken)
  );
  const bracePairs = [];
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token instanceof BinaryOperatorToken) {
      ...
      if (lastBrace) {
        tokens.splice(
          lastBrace.openBraceIndex,
          i - lastBrace.openBraceIndex + 1
        );
        tokens.splice(lastBrace.openBraceIndex, 0, new OpenBraceToken());
        tokens.splice(i, 0, new CloseBraceToken());
        if (
          bracePairs.length &&
          bracePairs[bracePairs.length - 1].minPrecedence >
            lastBrace.minPrecedence
        ) {
          bracePairs[bracePairs.length - 1].minPrecedence =
            lastBrace.minPrecedence;
        }
...
function flip(expression) {
  ...
  const factorizedTokens = factorOutBraces(tokens) || [
    new NumberToken("1"),
...
function hasBatsSymbols(expression) {
  const tokens = normalizeTokens(tokenize(expression));
  return tokens.some(
    (token) => token instanceof SymbolToken && token.exchange() === "BATS"
  );
}
...
function hasChxjpySymbols(expression) {
  const tokens = normalizeTokens(tokenize(expression));
  return tokens.some(
    (token) => token instanceof SymbolToken && token.exchange() === "CHXJPY"
  );
}
...
function hasFreeDelaySymbols(expression) {
  ...
  return tokens.some(
    (token) =>
      ...
      exchanges.includes((token || "").toUpperCase() + "_DLY")
  );
}

function getExchange(expression) {
  const tokens = normalizeTokens(tokenize(expression));
  const exchangeTokens = tokens
    .filter((token) => token instanceof SymbolToken)
    .map((token) => token.exchange());
...
function shortName(expression) {
  const tokens = factorOutBraces(normalizeTokens(tokenize(expression))) || [
    new NumberToken("1"),
    new BinaryOperatorToken(),
    new OpenBraceToken(),
    ...tokens,
