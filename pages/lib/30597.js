import { SessionInfo, isTradingNow } from "some-import-path"; // ! not correct
...
function processQuotesData(ticker, status) {
  ...
  TradingView.ChartapiMessagerInstances[currentTicker].onQuotesData([
    currentTicker,
    data,
  ]);
}
...
function updateMarketStatus() {
  if (currentTicker) {
    if (tickerTimer) {
      ...
      const status = isTradingNow(new Date().getTime(), tickerTimer)
        ? "market"
        : "out_of_session";
...
export default function (timezone, session, sessionHolidays, corrections) {
  ...
  function initialize(data) {
    if (!initialized) {
      ...
      if (
        currentTicker &&
        (currentTicker.ticker !== data.ticker ||
          currentTicker.timer !== secondsToNextMinute)
      ) {
